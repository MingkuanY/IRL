...
def main_image(uv, resolution):
    ...
    plane_pos = ray_origin + ray_direction * 0.15
    pos = np.floor(plane_pos[:2] * 300.0)  # Adjust the scaling factor
    #I think the 300.0 changes the size of checkerboard boxes
    

    #generate the content to be warped by the above effect. Here is an example with a checkerboard
    pattern_mask = np.mod(pos[0] + np.mod(pos[1], 2.0), 2.0)
    blue_color = np.array([0.0, 0.0, 1.0, 1.0])  # Blue
    purple_color = np.array([0.5, 0.0, 0.5, 1.0])  # Purple

    # Assign colors based on the pattern_mask
    color = blue_color if pattern_mask == 0 else purple_color
...
# Flip the image by 180 degrees and mirror it
output_image = np.flipud(concatenated_image)
